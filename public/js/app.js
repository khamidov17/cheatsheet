 ...
 * Improves performance by reducing the number of high-frequency calls (e.g., API requests while typing).
 *
...
/**
 * Internal history saving logic.
 * Parallelizes API calls and uses keepalive to ensure persistence.
 *
 * Performance impact: Parallelization reduces the total time spent waiting for network responses.
 * 'keepalive: true' ensures data is saved even if the user navigates away.
 */
async function saveHistoryInternal() {
    const promises = [];
    const payload = JSON.stringify(window.appState.history);
    ...
    if (window.appState.user && window.appState.user.email) {
        promises.push(
            fetch(`${API}/user/${encodeURIComponent(window.appState.user.email)}/history`, {
                ...
                body: payload,
                keepalive: true
            }).catch(err => console.warn('Failed to save user history', err))
        );
    }
    ...
    promises.push(
        fetch(`${API}/device/${window.appState.deviceId}/history`, {
            ...
            body: payload,
            keepalive: true
        }).catch(err => console.warn('Failed to save device history', err))
    );
...
/**
 * Debounced version of the history save.
 * Prevents excessive API calls during rapid typing.
 */
const saveHistoryDebounced = debounce(async () => {
    await saveHistoryInternal();
    // saveHistoryInternal already calls renderHistory() if landing view is active.
    // No extra renderHistory() needed here to avoid redundant DOM updates.
}, 1000);
...
/**
 * Saves the current state of the document being edited.
 * @param {boolean} immediate - If true, saves immediately without debounce.
 */
function saveCurrentDocument(immediate = false) {
    ...
    if (doc) {
        ...
        // If immediate is true, we flush any pending debounced saves to ensure consistency
        if (immediate) {
            saveHistory(true);
        } else {
            saveHistory();
...
function renderCanvasNodes() {
    ...
    window.appState.canvasData.forEach((section, idx) => {
        ...
        content.addEventListener('input', () => {
            ...
            saveCurrentDocument(); // Now debounced by default
        });
...
Do NOT wrap in \`\`\`json. Output the raw array only.`;
    ...
    try {
        ...
        openDocument(docId);
        saveHistory(true); // Save immediately on creation
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Flush pending changes when leaving page
    window.addEventListener('beforeunload', () => {
        if (saveHistoryDebounced.pending()) {
            saveHistory(true);
        }
    });
