...
/**
 * Debounces a function, ensuring it's only called after 'delay' ms of inactivity.
 * Includes .cancel(), .flush(), and .pending() for fine-grained control.
 * performance impact: Reduces frequency of expensive operations like network requests.
 */
...
async function saveDeviceHistory() {
    ...
    } catch (e) { console.error('Save device history failed:', e); }
}
...
/**
 * Handles the actual network requests for persistence.
 * performance impact: Uses Promise.all for parallel API calls.
 */
async function saveHistoryInternal() {
    ...
    // Only re-render history if we are actually looking at the landing page
...
/**
 * Debounced wrapper for history persistence.
 * @param {boolean} force - If true, flushes any pending save and executes immediately.
 */
...
/**
 * Updates the current document in state and triggers debounced persistence.
 * performance impact: Reduces redundant network calls and UI re-renders during active typing.
 */
