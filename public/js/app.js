...
function saveCurrentDocumentImmediate() {
...
function renderCanvasNodes() {
    ...
    window.appState.canvasData.forEach((section, idx) => {
        ...
        up.onclick = e => { e.stopPropagation(); if (idx > 0) { [window.appState.canvasData[idx], window.appState.canvasData[idx - 1]] = [window.appState.canvasData[idx - 1], window.appState.canvasData[idx]]; saveCurrentDocumentImmediate(); renderCanvasNodes(); updateWatermark(); } };

        const down = document.createElement('button');
        down.className = 'move-btn'; down.innerHTML = '▼';
        down.onclick = e => { e.stopPropagation(); if (idx < window.appState.canvasData.length - 1) { [window.appState.canvasData[idx], window.appState.canvasData[idx + 1]] = [window.appState.canvasData[idx + 1], window.appState.canvasData[idx]]; saveCurrentDocumentImmediate(); renderCanvasNodes(); updateWatermark(); } };

        const del = document.createElement('button');
        del.className = 'move-btn delete-btn'; del.innerHTML = '✕';
        del.onclick = e => { e.stopPropagation(); window.appState.canvasData.splice(idx, 1); saveCurrentDocumentImmediate(); renderCanvasNodes(); updateWatermark(); };
        ...
        w.addEventListener('drop', function (e) {
            ...
            if (si !== ti) { [window.appState.canvasData[si], window.appState.canvasData[ti]] = [window.appState.canvasData[ti], window.appState.canvasData[si]]; saveCurrentDocumentImmediate(); renderCanvasNodes(); updateWatermark(); }
        });
    ...
    addBtn.onclick = () => { window.appState.canvasData.push({ title: "My Notes", body: "<ul><li>Write here...</li></ul>" }); saveCurrentDocumentImmediate(); renderCanvasNodes(); updateWatermark(); };
        ...
        if (js !== -1 && je !== -1) {
            try {
                ...
                if (Array.isArray(sections) && sections.length > 0 && sections[0].title) {
                    window.appState.canvasData = sections; renderCanvasNodes(); saveCurrentDocumentImmediate(); updateWatermark();
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Back
    document.getElementById('back-btn').addEventListener('click', () => { saveCurrentDocumentImmediate(); toggleChat(false); switchToLanding(); });
