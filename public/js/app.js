...
function renderHistory() {
    ...
    [...window.appState.history].sort((a, b) => b.updatedAt - a.updatedAt).forEach(doc => {
        ...
        card.innerHTML = `<div class="history-title">${esc(doc.title)}</div><div class="history-date">${new Date(doc.updatedAt).toLocaleDateString()}</div>`;
...
function renderCanvasNodes() {
    ...
    window.appState.canvasData.forEach((section, idx) => {
        ...
        up.onclick = e => { e.stopPropagation(); if (idx > 0) { [window.appState.canvasData[idx], window.appState.canvasData[idx - 1]] = [window.appState.canvasData[idx - 1], window.appState.canvasData[idx]]; saveCurrentDocument(); renderCanvasNodes(); } };

        const down = document.createElement('button');
        down.className = 'move-btn'; down.innerHTML = '▼';
        down.onclick = e => { e.stopPropagation(); if (idx < window.appState.canvasData.length - 1) { [window.appState.canvasData[idx], window.appState.canvasData[idx + 1]] = [window.appState.canvasData[idx + 1], window.appState.canvasData[idx]]; saveCurrentDocument(); renderCanvasNodes(); } };

        const del = document.createElement('button');
        del.className = 'move-btn delete-btn'; del.innerHTML = '✕';
        del.onclick = e => { e.stopPropagation(); window.appState.canvasData.splice(idx, 1); saveCurrentDocument(); renderCanvasNodes(); };
        ...
        w.addEventListener('drop', function (e) {
            ...
            if (si !== ti) { [window.appState.canvasData[si], window.appState.canvasData[ti]] = [window.appState.canvasData[ti], window.appState.canvasData[si]]; saveCurrentDocument(); renderCanvasNodes(); }
        });
    ...
    addBtn.onclick = () => { window.appState.canvasData.push({ title: "My Notes", body: "<ul><li>Write here...</li></ul>" }); saveCurrentDocument(); renderCanvasNodes(); };
        ...
        if (js !== -1 && je !== -1) {
            try {
                ...
                if (Array.isArray(sections) && sections.length > 0 && sections[0].title) {
                    window.appState.canvasData = sections; renderCanvasNodes(); saveCurrentDocument();
