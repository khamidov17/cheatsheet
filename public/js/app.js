 ...
 * Performance Impact: Reduces redundant network requests and processing during high-frequency events.
 */
...
function saveDeviceHistory() {
    ...
    }).catch(e => console.error('Error in saveDeviceHistory:', e));
}
...
async function saveHistoryInternal() {
    ...
    if (window.appState.user && window.appState.user.email) {
        promises.push(fetch(`${API}/user/${encodeURIComponent(window.appState.user.email)}/history`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify(window.appState.history)
    ...
    // If the landing view is active, refresh the history grid
...
/**
 * Persists document history to the backend.
 * @param {boolean} immediate - If true, bypasses debounce and saves immediately.
 */
...
/**
 * Updates the current document in history and persists it.
 * @param {boolean} immediate - If true, saves immediately to the backend.
 */
...
function renderCanvasNodes() {
    ...
    window.appState.canvasData.forEach((section, idx) => {
        ...
        content.addEventListener('input', () => {
            ...
            saveCurrentDocument(); // Debounced save for typing
        });
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Ensure pending saves are flushed before leaving the page
