...
/**
 * Internal history saving logic with parallelization and keepalive for performance.
 * Optimizes network usage by firing both requests concurrently.
 */
async function saveHistoryInternal() {
    ...
    if (window.appState.user && window.appState.user.email) {
        promises.push(
            fetch(`${API}/user/${encodeURIComponent(window.appState.user.email)}/history`, options)
                .catch(err => console.warn('User history save failed:', err))
        );
    }

    promises.push(
        fetch(`${API}/device/${window.appState.deviceId}/history`, options)
            .catch(err => console.warn('Device history save failed:', err))
    );
...
// Performance: Debounced history save to prevent network congestion during rapid edits
const saveHistoryDebounced = debounce(saveHistoryInternal, 1000);

// Unified saveHistory function
...
/**
 * Persists the current document state to history.
 * @param {boolean} [immediate=false] - Whether to save immediately (true) or use debouncing (false).
 */
function saveCurrentDocument(immediate = false) {
    ...
    if (doc) {
        ...
        if (immediate) {
            // If there's a pending debounced save, flush it to save now.
            // If not, trigger an immediate save.
            if (saveHistoryDebounced.pending?.()) {
                saveHistoryDebounced.flush();
            } else {
                saveHistoryInternal();
            }
        } else {
...
function renderHistory() {
    ...
    // Performance: skip rendering if landing view is not active
    if (!document.getElementById('landing-view')?.classList.contains('view-active')) {
        return;
    }
