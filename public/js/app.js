 ...
 * Optimization: Reduces network traffic and server load by batching frequent updates.
 * Impact: Significant reduction in API calls during active editing.
 */
 ...
 * Optimization: Parallelizes User and Device API calls using Promise.all to reduce total latency.
 */
...
/**
 * Public API to save history.
 * Optimization: Uses debouncing by default to avoid excessive network requests during typing.
 ...
 * Optimization: Uses conditional rendering of the history grid to avoid layout thrashing during typing.
 * @param {boolean} force - If true, persists immediately and refreshes the history grid.
 */
function saveCurrentDocument(force = false) {
    ...
    if (doc) {
        ...
        // Only re-render history if we are in the landing view or if it's a forced save
        const landingView = document.getElementById('landing-view');
        if (force || (landingView && landingView.classList.contains('view-active'))) {
            renderHistory();
        }
    }
}
...
document.addEventListener('DOMContentLoaded', () => {
    // Optimization: Ensure pending debounced history is flushed before page exit.
