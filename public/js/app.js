...
/**
 * Internal function to save history to both user and device endpoints in parallel.
 * Performance: Parallelizing these calls reduces the total persistence time from
 * (User latency + Device latency) to max(User latency, Device latency).
 */
async function saveHistoryInternal() {
    ...
    // Only render history if the landing view is active to avoid redundant DOM work
    renderHistory();
}

/**
 * Debounced version of history saving to prevent excessive API calls during rapid edits.
 * 1000ms delay provides a good balance between responsiveness and efficiency.
 */
const debouncedSaveHistory = debounce(saveHistoryInternal, 1000);

/**
 * Public saveHistory function that supports both debounced and immediate saving.
 * @param {boolean} force - If true, triggers an immediate save and flushes any pending debounced tasks.
 */
...
/**
 * Saves the current document state to the history array.
 * @param {boolean} force - If true, triggers an immediate, non-debounced save to the backend.
 * Performance: Uses debounced saving by default to optimize network traffic during typing.
 */
 ...
 * Performance: Skips DOM updates if the landing view is not active.
 */
function renderHistory() {
    const landingView = document.getElementById('landing-view');
    if (!landingView || !landingView.classList.contains('view-active')) return;
...
function switchToLanding() {
    ...
    landing.classList.replace('hidden', 'view-active');
    removeWatermark();
    renderHistory();
}
...
document.addEventListener('DOMContentLoaded', () => {
    // Flush debounced saves before user leaves to ensure state consistency
