...
const saveHistory = debounce(_saveHistory, 1000);
...
function saveCurrentDocument(immediate = false) {
    ...
    if (doc) {
        ...
        // Only re-render history if we are not in the editor (the grid is hidden)
        const landingActive = document.getElementById('landing-view').classList.contains('view-active');
        if (landingActive || immediate) renderHistory();
    }
}
