...
/**
 * Debounce utility to prevent excessive execution of high-frequency events.
 * Optimization: Reduces CPU and Network usage by batching calls.
 */
...
/**
 * Optimized persistence: Saves history to both User and Device in parallel.
 * Uses keepalive: true to ensure the request completes even if the page is closed.
 */
async function saveHistoryInternal() {
    ...
    if (window.appState.user && window.appState.user.email) {
        requests.push(fetch(`${API}/user/${encodeURIComponent(window.appState.user.email)}/history`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: historyData,
            keepalive: true
    ...
    requests.push(fetch(`${API}/device/${window.appState.deviceId}/history`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: historyData,
        keepalive: true
    ...
    // Refresh history UI if on landing page
 ...
 * @param {boolean} immediate - If true, saves immediately and refreshes history UI.
 */
function saveCurrentDocument(immediate = false) {
    ...
    if (doc) {
        ...
        // Only re-render history immediately if requested or if not active typing
        if (immediate) renderHistory();
    }
}
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Performance Optimization: Save on window close
