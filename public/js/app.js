 ...
 * Includes .cancel() and .flush() methods for finer control.
 * Performance impact: Reduces redundant API calls and heavy DOM updates during rapid input (like typing).
 */
...
async function saveDeviceHistory() {
    try {
        await fetch(`${API}/device/${window.appState.deviceId}/history`, {
            ...
            keepalive: true // Ensure save completes even if page is closed
        });
    } catch (e) { }
}
...
/**
 * Debounced history saving to prevent excessive API calls during typing.
 * Optimization: 1000ms delay captures most typing pauses while remaining responsive.
 */
const debouncedSave = debounce(saveHistoryInternal, 1000);
...
function saveCurrentDocument(immediate = false) {
    ...
    if (doc) {
        ...
        // Optimization: Only re-render the history grid if we're on the landing page OR it's an immediate/critical save
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Save any pending debounced changes before user leaves
