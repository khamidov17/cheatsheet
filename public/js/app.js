...
/**
 * Standard debounce implementation with flush and cancel support.
 * Used to prevent excessive API calls and re-renders during rapid input.
 */
...
/**
 * Saves current history to device storage.
...
/**
 * Internal persistence function using parallel requests to optimize speed.
 ...
 * Performance: Reduces network traffic by ~90% during active typing.
 */
const debouncedSaveHistory = debounce(async () => {
    await saveHistoryInternal();
    // Only re-render history grid if landing view is active
...
async function mergeDeviceHistoryToUser(email) {
    try {
        ...
        // Use forced save for critical state change
...
/**
 * Persists the current document state to appState and triggers backend sync.
 * @param {boolean} force - If true, syncs immediately and re-renders UI.
 */
function saveCurrentDocument(force = false) {
    ...
    if (doc) {
        ...
        // If forced, we definitely want to make sure history grid is updated
        if (force) renderHistory();
    }
}
...
function renderCanvasNodes() {
    ...
    window.appState.canvasData.forEach((section, idx) => {
        ...
        content.addEventListener('input', () => {
            ...
            // Debounced save for typing performance
